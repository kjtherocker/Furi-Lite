...
class RedBossObject : public BossObject
{
protected:
    float m_TestTimer;
    ...
    typedef void(RedBossObject::*BossState)(float delta);

    BossState m_BossState = 0;
...
public:
    RedBossObject(Scene* pScene, std::string name, vec3 pos, vec3 rot, vec3 scale, Mesh* pMesh, Material* pMaterial, Game* game);
    RedBossObject(Scene* pScene, std::string name, vec3 pos, vec3 rot, vec3 scale, std::string meshname, std::string materialname, Game* game);
    virtual ~RedBossObject();
    ...
    RedBossObject::BossState GetBossState();
};
