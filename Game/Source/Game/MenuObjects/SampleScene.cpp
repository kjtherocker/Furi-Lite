...
void SampleScene::LoadContent()
{
    ...
    {    
        ...
        // Camera
        m_pGameObjects["Camera"] = new CameraObject(this, "Camera", vec3(0, 0, -100), vec3(0, 0, 0), vec3(1, 1, 1));
        ((CameraObject*)m_pGameObjects["Camera"])->SetPlayerObject(((PlayerObject*)m_pGameObjects["Player"]));
		//TextureGrey
		//TextureFireGround
        m_pGameObjects["Ground"] = new GameObject(this, "Ground", vec3(0, 0, 7), vec3(0, 0, 0), vec3(13, 13, 1), "Obj", "TextureFireGround", m_pGame);
        m_pGameObjects["Ground"]->SetRenderOrder(1);

        

		m_pGameObjects["Reflector"] = new GameObject(this, "Reflector", vec3(0, -3, 10), vec3(0, 0, 0), vec3(0.2f), "Sphere", "TextureSkyBoxReflection", m_pGame);

       

		//m_pGameObjects["BlueBossWall"] = new GameObject(this, "BlueBossWall", vec3(-5, -5, 0), vec3(90, 0, 0), vec3(2, 5, 1), "Plane", "TextureRedBossWall", m_pGame);
       ...
       // m_pGameObjects["FBos"] = new GameObject(this, "FBO", vec3(-10, -10, 3), vec3(90, 0, 0), vec3(5, 40, 1), "Plane", "FBOTexture", m_pGame);
    

		m_pGameObjects["SlantedCube"] = new GameObject(this, "SlantedCube", vec3(0, 0, 5), vec3(90, 0, 0), vec3(1, 1, 1), "Slantedcube", "TextureRedAreanaWall", m_pGame);



        m_pGameObjects["WallTop"] = new GameObject(this, "WallTop", vec3(0, 9, 0), vec3(90, 0, 0), vec3(10, 1, 1), "Obj", "TextureRedAreanaWall", m_pGame);
        m_pGameObjects["WallTop"]->AddPhysicsBox(vec2(25, 1), true, true, 1, false);
        m_pGameObjects["WallTop"]->SetRenderOrder(4);

		m_pGameObjects["WallBottom"] = new GameObject(this, "WallBottom", vec3(0, -9, 0), vec3(0, 0, 0), vec3(10, 1, 1), "Obj", "TextureRedAreanaWall", m_pGame);
		m_pGameObjects["WallBottom"]->AddPhysicsBox(vec2(25, 1), true, true, 1, false);
		m_pGameObjects["WallBottom"]->SetRenderOrder(4);

		m_pGameObjects["WallLeft"] = new GameObject(this, "WallLeft", vec3(-9, 0, 0), vec3(0, 0, 0), vec3(1, 8, 1), "Obj", "TextureRedAreanaWall", m_pGame);
		m_pGameObjects["WallLeft"]->AddPhysicsBox(vec2(25, 1), true, true, 1, false);
		m_pGameObjects["WallLeft"]->SetRenderOrder(4);

		m_pGameObjects["WallRight"] = new GameObject(this, "WallRight", vec3(9, 0, 0), vec3(0, 0, 0), vec3(1, 8, 1), "Obj", "TextureRedAreanaWall", m_pGame);
       ...
       /* m_pGameObjects["BlueBossWall"]->AddPrismaticJoint(((GameObject*)m_pGameObjects["BlueBossWall"])->GetPosition().x, ((GameObject*)m_pGameObjects["BlueBossWall"])->GetPosition().y, b2Vec2(-1.0f, 0.0f),
        m_pGameObjects["BlueBossWall"]->GetPhysicsBody(), this->GetBox2DWorld()->GetGroundBody(), true, -8, 8);
*/
...
void SampleScene::Update(float deltatime)
{
    ...
    m_pGame->GetSceneManager()->SetRenderScene(m_pGame->GetSceneManager()->GetScene("SampleScene"));
    ...
    if (((RedBossObject*)m_pGameObjects["RedBoss"])->GetHealth() <= 0)
    {
	...
	//m_pGameObjects["BlueBossWall"]->GetPhysicsBody()->ApplyForce(b2Vec2(200, 0), b2Vec2(0.0f, 0.0f), true);
}
