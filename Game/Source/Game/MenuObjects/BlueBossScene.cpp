...
BlueBossScene::BlueBossScene(Game* pGame, ResourceManager* pResources, Framework * pframework)
: Scene( pGame, pResources, pframework)
{
}

BlueBossScene::~BlueBossScene()
{
	delete m_pSkyBoxObject;
}

void BlueBossScene::OnSurfaceChanged(unsigned int width, unsigned int height)
{
    Scene::OnSurfaceChanged( width, height );
}

void BlueBossScene::LoadContent()
{
	Scene::LoadContent();
	...
	// Create some game objects.
	{
		...
		m_pGameObjects["BlueBossWall"]->AddPhysicsBox(vec2(1, 1), true, true, 1, false);
...

	{
		...
		vec3 m_LightPos[5] = { vec3(8, 8, -2),  vec3(-8, -8, -2), vec3(-8, 8, -2), vec3(8, -8, -2), ((BlueBossObject*)m_pGameObjects["BlueBoss"])->GetPosition() };
		...
		// 4 - Player
		{
			...
			m_pGameObjects["BlueBoss"]->SetRenderOrder(3);
		}
	}
}

void BlueBossScene::OnEvent(Event * pEvent)
{
...
void BlueBossScene::Update(float deltatime)
{

	Scene::Update(deltatime);

	m_pGame->GetSceneManager()->SetRenderScene(m_pGame->GetSceneManager()->GetScene("BlueBoss"));
	...
	if (((BlueBossObject*)m_pGameObjects["BlueBoss"])->GetHealth() <= 0)
	{
		m_pGame->SetBlueBossKilled(1);
	...
	//Lights
	m_LightPos[4] = ((BlueBossObject*)m_pGameObjects["BlueBoss"])->GetPosition();
	m_pMesh->SetLightInfo(m_NumberOfLights, m_LightPos, m_LightColor);
	vec3 lightpos = vec3(8, 8, 5);
	((BlueBossObject*)m_pGameObjects["BlueBoss"])->SetBossPos(lightpos);
	// Mouse
	...
	if (((PlayerObject*)m_pGameObjects["Player"])->GetIsShooting() == true)
	{
		...
		if (raycastresult.m_Hit == true)
		{
			if (raycastresult.m_pBody->GetUserData() == m_pGameObjects["BlueBoss"])
			{
				...
				((BlueBossObject*)m_pGameObjects["BlueBoss"])->ChangeHealth(3);
