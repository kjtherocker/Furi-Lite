...
LRESULT CALLBACK Framework::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_RBUTTONDOWN:
    {
        ...
        InputEvent* pEvent = new InputEvent(InputDeviceType_Mouse, InputState_Pressed, 1, vec2((float)x, (float)y));
        pFramework->m_pGameCore->QueueEvent(pEvent);
    }
    return 0;
    ...
    case WM_RBUTTONUP:
    {
        pFramework->m_MouseButtonStates[1] = false;

        int x = GET_X_LPARAM(lParam);
        int y = GET_Y_LPARAM(lParam);

        InputEvent* pEvent = new InputEvent(InputDeviceType_Mouse, InputState_Released, 1, vec2((float)x, (float)y));
        pFramework->m_pGameCore->QueueEvent(pEvent);
    }
    return 0;
